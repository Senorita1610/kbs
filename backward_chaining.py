...
class BackwardChaining:
    ...
    # trả về giá trị true false cập nhật cho result
    def do_backward_chaining(self, goal, indent=""):
        ...
        for rule in self.rules:
            # ls=0
            dk = 1  # Biến điều kiện để dừng vòng lặp khi có thông tin không thuộc fact ban đầu
            ...
            if rule.right == goal:
                ...
                for f in range(len(rule.left)):
                    ...
                    if fact_guess not in self.target_facts:
                        self.print_step(
                            fact_guess, indent + '-', "Không có luật nào để suy diễn/không có thông tin này ban đầu. Trả về thất bại.")
                        dk = 0
                        break
                    else:
                        self.print_step(fact_guess, indent + "-", "Cập nhật thông tin %s, bởi vì được tìm thấy trong tập thông tin gốc %s. Trả về thành công." % (
                                        fact_guess, ", ".join(self.target_facts)))

                        # dk=1
                self.road = "R" + str(self.rules.index(rule) + 1)

                if dk == 1:  # Kiểm tra nếu đúng hết các thông tin thì dừng vòng lặp
                    ...
                    break
        if ls == 0:  # Nếu không có luật nào trả lời đúng theo fact thì dừng
            self.print_step(
                goal, indent, "Không có luật nào để suy diễn/không có thông tin này ban đầu. Trả về thất bại.")
            return False
        else:
            return True
    ...
    def print_step(self, goal, indent, msg):
        self.iteration += 1
        self.output += str(self.iteration).rjust(3, " ") + \
            ") %sGoal %s. " % (indent, goal) + msg + "\n"
    ...
    def print_data(self, rules, facts, goal):
        ...
        self.output += "\n  2) Thông tin người dùng đang có\n    %s.\n\n" % ", ".join(
            facts)
        self.output += "  3) Lịch tập dự đoán\n    %s.\n\n" % goal
    ...
    def write_output(self):
        self.output_file_name = "bc_output.txt"
