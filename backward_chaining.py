...
class BackwardChaining:
    ...
    def do_backward_chaining(self, goal, indent=""):
            ...
            if rule.right == goal:
                ...
                for f in range(len(rule.left)):
                    ...
                    if fact_guess not in self.target_facts:
                        self.print_step(
                            fact_guess, indent + '-', "Không có luật nào để suy diễn/không có triệu chứng này ban đầu. Trả về thất bại.")
                        dk = 0
                        break
                    else:
        ...
        if ls == 0:  # Nếu không có luật nào trả lời đùng theo fact thì dừng
            self.print_step(
                goal, indent, "Không có luật nào để suy diễn/không có triệu chứng này ban đầu. Trả về thất bại.")
            return False
        else:
            return True
